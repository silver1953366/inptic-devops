...
GMAIL_USER = "jeuxm0703@gmail.com"  # Remplace par ton adresse Gmail
GMAIL_PASS = "wnnahwqvwlzpxjdw"     # Ton code de 16 caractères
DESTINATAIRE = "jeuxm0703@gmail.com" # L'adresse qui recevra les alertes

def envoyer_alerte(sujet, message_corps):
    msg = MIMEMultipart()
    msg['From'] = GMAIL_USER
    msg['To'] = DESTINATAIRE
    msg['Subject'] = sujet
...
<body>
    ...
    <div class="main">
        ...
        <div class="status-bar">
            ...
            <span style="font-size: 0.9rem;">Notifications Email & Prometheus Actifs</span>
        </div>
...
def save():
    ...
    if uid:
        ...
        envoyer_alerte("✏️ Modification Étudiant", f"L'étudiant ID #{uid} a été mis à jour : {nom} {prenom} ({fil})")
    else:
        ...
        envoyer_alerte("✅ Nouvelle Inscription", f"Nouvel étudiant enregistré : {nom} {prenom} en {fil}")
...
def delete(uid):
    db = get_students()
    # On récupère le nom pour le mail avant de supprimer
    target = next((s for s in db if s['id'] == uid), None)
    ...
    envoyer_alerte("⚠️ Suppression Étudiant", f"L'étudiant {nom_supprime} (ID #{uid}) a été supprimé de la base.")
    return redirect(url_for('home'))
